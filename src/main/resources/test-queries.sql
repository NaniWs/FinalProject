...
-- 4. Все клиенты, сделавшие более одного заказа
SELECT
    c.id,
    c.first_name,
    c.last_name,
...
------------------------------------------------------------
-- 6. Обновление количества товара на складе при покупке (эмуляция покупки)
-- Уменьшаем количество на складе товара id=1 на 2 шт
UPDATE products
SET quantity = quantity - 2
WHERE id = 1 AND quantity >= 2;

-- 7. Изменение статуса заказа (например, заказ id=1 переведён в "Доставлен")
UPDATE orders
...
-- 10. Удаление заказов старше 1 года (архивная очистка)
DELETE FROM orders
